...
- Entity or concept definitions outside `glossary/glossary.yaml` (or the equivalent glossary L3 under the resolved category path). The glossary L3 is the single source of truth; reference entities by name from other L2 files, do not redefine them. If a term is missing, add it to the glossary first.
