...
7. **IDs are timestamps.** All L3 entries use the format `{type}_{YYYYMMDD}_{HHMMSS}_{4hex}` where `{type}` is one of `adr`, `rv`, `gotcha`, `pb`. This makes IDs sortable and collision-free.
