...
7. **IDs are timestamps.** All L3 entries use the format `{type}_{YYYYMMDD}_{HHMMSS}_{8hex}` where `{type}` is one of `adr`, `rv`, `gotcha`, `pb`. The 8-hex suffix is random (32 bits), giving collision-resistance even when multiple entries are appended within the same second.
