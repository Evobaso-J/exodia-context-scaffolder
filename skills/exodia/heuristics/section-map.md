...
| ---------------------------------------------------- | -------- |
| `architecture`, `routing`, `router`, `module`, `ssr`, `csr`, `bundle`, `chunk`, `state management`, `store`, `runtime`, `entry point`, `build pipeline`, `entity`, `entities`, `data model`, `schema` (when data-schema, not GraphQL-schema pattern), `type system`, `orm`, `prisma`, `openapi` (as schema source) | `architecture` |
...
| `domain`, `glossary`, `terminology`, `user journey`, `business logic`, `business rule`, `domain event`, `model` (only if clearly business-domain model, not data model or ML) | `domain` |
