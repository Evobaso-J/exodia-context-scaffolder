...
- **Sibling notice (config-driven only).** For each parent directory shared by two or more managed paths (e.g. `docs/handbook/` is parent to a managed `glossary/` plus user-owned `onboarding/` and `tech/`), list the unmanaged sibling dirs grouped per parent: `Note: docs/handbook/ has 2 unmanaged sibling dirs (onboarding/, tech/) — left untouched.`. Use `:` or `;` instead of `—` if the surrounding doc style avoids em-dashes.
